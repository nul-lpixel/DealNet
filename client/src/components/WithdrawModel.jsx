...
const WithdrawModel = ({onclose}) => {
    ...
    const handleSubmission=async(e)=>{
        ...
        try {
            ...
            if(account.length===0) return toast.error("Please add atleast one field");

            //check all fields are filled
            for(const field of account){
                if(!field.value) return toast.error(`Please fill the ${field.name} field`)
            }

            const confirm=window.confirm("Are you sure want to submit?")
            if(!confirm) return;
        ...
        <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-auto flex flex-col'>
            ...
            <div className='bg-gradient-to-r from-indigo-600 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between'>
                <div className='flex-1 min-w-0'>
                    <h3 className='font-semibold text-lf truncate'>Withdraw Funds</h3>
                </div>
                <button onClick={onclose} className='ml-4 p-l hover:bg-white/20 hover:bg-opacity-20 rounded-lg transition-colors'>
                    <X className='w-5 h-5'/>
                </button>
            </div>

            {/* Form */}
            <form onSubmit={handleSubmission} className='flex flex-col items-start gap-5 p-4 overflow-y-scroll'>
