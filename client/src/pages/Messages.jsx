...
const Messages = () => {
  ...
  return chats.filter((chat) => {
    const chatUser = chat.chatUserId === user?.id ? chat.ownerUser : chat.chatUser;
    const title = chat.listing?.title || '';
    const name = chatUser?.name || '';
    const last = chat.lastMessage || '';
    
    return `${title} ${name} ${last}`.toLowerCase().includes(q);
  });
      ...
      <div className='py-10'>        
        ...
        <div className='relative max-w-xl mb-8'> 
          ...
          <input type="text" placeholder='Search conversations...' value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)}
           className='w-full p-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-indigo-500'
           />
        </div>
